...
float temp;
float airTemp;     // should be air temp
float highestTemp;
int tempInterval = 5.5;
...
void setup() {
  ...
  airTemp = awesome.temperatureSensor.read() - 0.25;
  highestTemp = airTemp + tempInterval;
}
...
void loop() {
  ...
  howHotIsIt = (temp - airTemp ) / ( highestTemp - airTemp );
